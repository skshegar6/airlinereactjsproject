...
class PassangerForm extends React.Component{
    ...
    async componentDidMount(){
        ...
        if(this.props.match.params.passengerId){
            if(this.props.passenger && this.props.passenger.length > 0 ){
               ...
               let passengerData =  this.props.passenger.filter((value) =>{
                    if(value.flight_id == flightId) return value;
                })
                this.flightdata1 =  this.passagerFlightMapData(passengerData,flightId); 
               let passengerByflightData = passengerData.filter((data) =>{
    ...
    addSeatCallbackContinousCase = ({ row, number, id }, addCb, params, removeCb) => {
        this.setState({
          ...
          await new Promise(resolve => setTimeout(resolve, 750))
          console.log(`Added seat ${number}, row ${row}, id ${id} ------`+ this.state.name)
          const newTooltip = `tooltip for id-${id} added by callback`
          let seat_checkIn_details={'id': id, 'number': number, isReserved: false,'tooltip':'Reserved by '+this.state.name+' and Not CheckIn','seat_array_index':this.getIndexdetails(row, number)};
          ...
          this.setState({seat_no:`row ${row},${number}`,seat_status:true,seat_details:seat_details,seat_checkIn_details:seat_checkIn_details})
    ...
    async submitFormData(evt){
        ...
        if(this.state.seat_status){
            ...
            seat_checkIn_details.tooltip = 'Reserved by '+this.state.name+' and Not CheckIn';
        ...
        await this.props.actions.savePassangerInServer(this.state);
        this.props.history.push('/admin/passanger/'+this.state.flight_id);
    }
    ...
    render(){
        ...
        <div>   
            ...
            <div className="col-md-6" style={{'textAlign':'center'}}>
             <div style={{ marginTop: '70px' }}>
              <div className="MapSeated" style={{'float':'right','width':'100px'}}><div style={{'width': '90px','float':'left'}}>
                    ...
                    ></div><div style={{'paddingRight':'5px'}}>avilable</div></div>
                    <div style={{'width': '90px','float':'left'}}>
                    <div style={{'height': '20px',
                                    'width': '15px',
                                    'backgroundColor': '#4CAF50',
                                    'float':'left'}}>
                    </div><div style={{'float':'left','paddingLeft':'5px'}}>selected</div></div>
                    <div style={{'width': '90px'}}>
                    <div style={{'height': '20px',
                                    'width': '15px',
                                    'backgroundColor': '#E0E0E0',
                                    'float':'left'}}>
                    </div><div style={{'float':'left','paddingLeft':'5px'}}>reserved</div></div>
                </div> 
            ...
            <form action="/action_page.php" style={{ marginTop: '80px' }}>
                ...
                <div className="form-group">
                    Seat Details: <input type="text" className="form-control" name="dob" value={this.state.seat_no} disabled onChange={this.onChangeData.bind(this)}/>
                </div>
                <div className="form-group">
                ...
                <div className="row">
                    <div className="col-md-12" style={{'textAlign':'center'}}>
                        ...
                        <Link id="link" className='btn btn-primary' style={{'marginRight':'10px','display':'none'}} to={'/admin/passanger/'+this.state.flight_id} >Home</Link>
                    </div>
                </div>
            </form>
            </div>
